...
# https://github.com/baklai/privoxy-tor-proxy/blob/main/README.md
...
for i in $(seq 1 20); do
    ...
    if [ $i -eq 20 ] ; then
        echo -e "\nTimed out waiting for proxy start, consult your container logs for more info (\`docker logs privoxy-tor-proxy\`)"
        exit 1
    fi
done;